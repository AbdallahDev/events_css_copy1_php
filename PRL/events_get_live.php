...
if ($rs->num_rows > 0) {
    if ((date('H:i') >= date('08:00')) && (date('H:i') <= $row_event_max_time['time'])) {
        ?> <table style="font-size: <?php echo $row_table_live_design['table_live_design_font_size']; ?>px; background: rgba(0, 0, 0, 0.69);">
            <tr>
                <th>النشاط</th>
                <th>الوقت</th>
                <th>المكان</th>
                ...
                <tbody style="vertical-align: text-top; text-align: center; ">
                    <tr>
                        <td style="width: <?php echo $row_table_live_design['table_live_design_event_entity_column_width'] ?>%"><?php
                            ...
                            ?></td>
                        <td style="width: <?php echo $row_table_live_design['table_live_design_event_time_column_width']; ?>%; direction: ltr"><?php echo date('h:i A', strtotime($row['time'])); ?></td>
